...
//#define VAR_bFirstEnter new CExpr(NULL, EXPR_TYPE_PTR_ELEMENT, new CExpr(NULL, EXPR_TYPE_TOKEN_WITH_NAMESPACE, LOCALVAR_NAME), pVarDef->getName())
	...
	case STATEMENT_TYPE_FLOW_TRY:
	{
		...
		CFunction* pSubFunc = new CFunction(m_pFunc->getParent()->getRealScope(), sub_func_name, pFuncType, FLOW_TYPE_FLOW,  
			new CFuncDeclare(m_pFunc->getParent()->getRealScope(), sub_func_name, pFuncType));
