...
public:
    ...
    bool findVar(const std::string& name) { return m_var_list.find(name) != m_var_list.end(); }
    void addVar(const std::string& name) { m_var_list.insert(name); }
...
int main(int argc, char* argv[])
{
    ...
    loop();
	// we cannot call flow_end here because if this flow is within a flow_try/flow_catch then it will be deleted twice
    //__flow_end(pFlow);
    return ret;
}
