...
bool g_log_impl = false;
#define TRACE(fmt)	do { if (g_log_impl) printf(fmt); } while (false)
#define TRACE2(fmt, ...)	do { if (g_log_impl) printf(fmt, __VA_ARGS__); } while (false)
...
{
	TRACE2("flow_cancel_wait, pObj=0x%lx, callback_func=0x%lx, waiting_flow=0x%lx\n", (unsigned long)pObj, (unsigned long)pObj->callback_func, (unsigned long)pObj->waiting_flow);
